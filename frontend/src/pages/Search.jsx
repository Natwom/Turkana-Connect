...
const Search = () => {
  ...
  const fetchSortedSongs = async (sortType) => {
    setLoading(true)
    try {
      const res = await songsApi.getSongs({ sort: sortType, limit: 50 })
  ...
  const getSortTitle = () => {
    switch (sort) {
      ...
      case 'newest': return 'New Releases'
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      >
        ...
        <div className="relative pt-8 pb-6 px-4">
          ...
          {isSortMode && (
            ...
            >
              <button 
                onClick={() => { setSort(''); setSearchParams({}); }}
                className="p-2 rounded-xl bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white transition-colors"
              >
                <ArrowLeft className="w-5 h-5" />
              </button>
              <div className="flex items-center gap-2">
                {sort === 'trending' && <TrendingUp className="w-5 h-5 text-primary" />}
                {sort === 'newest' && <Clock className="w-5 h-5 text-yellow-500" />}
                {sort === 'likes' && <Heart className="w-5 h-5 text-red-500" />}
      ...
      <div className="px-4">
        ...
        {!loading && !hasResults && (query || isSortMode) && (
          <motion.div 
          ...
          >
            ...
            <h3 className="text-2xl font-bold text-white mb-2">
              {isSortMode ? 'No songs found' : 'No results found'}
            </h3>
            <p className="text-gray-400 max-w-md mb-6">
              {isSortMode 
                ? 'There are no songs available in this category yet.'
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {results.playlists.map((playlist, i) => (
                          ...
                          >
                            ...
                            <p className="text-xs text-gray-500">{playlist.track_count || '0'} tracks</p>
                          </motion.div>
                        ))}
                      </div>
                    </motion.section>
